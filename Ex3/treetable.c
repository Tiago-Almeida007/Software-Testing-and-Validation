...
int balanced(TreeTable *t) {
    return check_balanced(t->root, t) != -1;
}


static int check_balanced(RBNode *rbn, TreeTable *t) {
    if (rbn == NULL || rbn == t->sentinel) return 0;
...
static int rbnode_height(RBNode *rbn, TreeTable *t){
    ...
    if(rbn == NULL|| rbn == t->sentinel){return 0;}
    //Check left(height)
    int left_height = rbnode_height(rbn->left, t);
    //Check right (height)
    int right_height = rbnode_height(rbn->right, t);
    
    //Return height of current + 1
    return (left_height >= right_height? left_height : right_height) + 1;
}

//Return 1 if the tree table is key sorted, 0 otherwise
//return 1 when for any node, its key is greater than all keys in the left
